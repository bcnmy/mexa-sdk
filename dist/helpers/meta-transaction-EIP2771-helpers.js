...
exports.getDomainSeperator = exports.buildForwardTxRequest = exports.findTheRightForwarder = void 0;
// import txDecoder from 'ethereum-tx-decoder';
const ethers_1 = require("ethers");
const abis_1 = require("../abis");
const config_1 = require("../config");
const utils_1 = require("../utils");
const findTheRightForwarder = (findTheRightForwarderParams) => __awaiter(void 0, void 0, void 0, function* () {
    const { to, smartContractTrustedForwarderMap, provider, forwarderAddresses, forwarderAddress, } = findTheRightForwarderParams;
    let forwarderToUse;
    if (smartContractTrustedForwarderMap[to]) {
        forwarderToUse = smartContractTrustedForwarderMap[to];
    }
    else {
        const contract = new ethers_1.ethers.Contract(to, abis_1.eip2771BaseAbi, provider);
        ...
        let forwarder;
        try {
            forwarder = yield contract.trustedForwarder();
        }
        catch (error) {
            (0, utils_1.logMessage)("Could not find read method 'trustedForwarder' in the contract abi");
        ...
        for (let i = 0; i < supportedForwarders.length; i += 1) {
            ...
            try {
                ...
                const isTrustedForwarder = yield contract.isTrustedForwarder(supportedForwarders[i]);
                ...
                (0, utils_1.logMessage)("Could not find read method 'isTrustedForwarder' in the contract abi");
                (0, utils_1.logMessage)(JSON.stringify(error));
            }
        }
    }
...
exports.findTheRightForwarder = findTheRightForwarder;
const buildForwardTxRequest = (account, to, gasLimitNum, data, biconomyForwarder, batchId = 0) => __awaiter(void 0, void 0, void 0, function* () {
    if (!biconomyForwarder) {
        throw new Error('Biconomy Forwarder is not defined for current network');
    }
    const batchNonce = yield biconomyForwarder.getNonce(account, batchId);
    const req = {
        from: account,
        to,
        token: config_1.config.ZERO_ADDRESS,
        txGas: gasLimitNum,
        tokenGasPrice: '0',
        batchId,
...
const getDomainSeperator = (biconomyForwarderDomainData) => {
    const domainSeparator = ethers_1.ethers.utils.keccak256(ethers_1.ethers.utils.defaultAbiCoder.encode([
        'bytes32',
        'bytes32',
        'bytes32',
        'address',
        ...
        ethers_1.ethers.utils.id('EIP712Domain(string name,string version,address verifyingContract,bytes32 salt)'),
        ethers_1.ethers.utils.id(biconomyForwarderDomainData.name),
        ethers_1.ethers.utils.id(biconomyForwarderDomainData.version),
...
//# sourceMappingURL=meta-transaction-EIP2771-helpers.js.map