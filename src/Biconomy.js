...
async function sendSignedTransaction(engine, payload, end) {
    if (payload && payload.params[0]) {
        ...
        if (typeof data == "string") {
            ...
            rawTransaction = data;
        } else if (typeof data == "object") { // Here user wrapped raw Transaction in json object along with signature
                ...
                if (! methodInfo) {
                    ...
                    if (! methodInfo) {
                        if (engine.strictMode) {
                            let error = formatMessage(RESPONSE_CODES.DASHBOARD_DATA_MISMATCH, `No smart contract wallet or smart contract registered on dashboard with address (${
                                decodedTx.to
                            })`);
                            return end(error);
                        } else {
         ...
         */
                ...
                let error = formatMessage(RESPONSE_CODES.INVALID_PAYLOAD, `Not able to deode the data in rawTransaction using ethereum-tx-decoder. Please check the data sent.`);
                eventEmitter.emit(EVENTS.BICONOMY_ERROR, error);
                end(error);
            }
        } else {
                ...
                if (engine.isLogin) {
                    ...
                    if (! gasLimit || parseInt(gasLimit) == 0) {
                        ...
                        if (contractABI) {
                            let web3 = getWeb3(engine);
                            let contract = new web3.eth.Contract(JSON.parse(contractABI), to);
                            gasLimit = await contract.methods[methodName].apply(null, paramArrayForGas).estimateGas({from: userContractWallet});
                        }
                    }
                    ...
                    engine.send({
                        ...
                        _logMessage(`User signature for payload id ${
                            payload.id
                        } is ${
                            response.result
                        }`);
...
eventEmitter.on(EVENTS.SMART_CONTRACT_DATA_READY, (dappId, engine) => { // Get DApp API information from Database
    let getAPIInfoAPI = `${baseURL}/api/${
        config.version
    }/meta-api`;
    fetch(getAPIInfoAPI, getFetchOptions("GET", engine.apiKey)).then((response) => response.json()).then(function (response) {
...
async function _sendTransaction(engine, account, api, data, cb) {
    if (engine && account && api && data) {
        ...
        fetch(`${baseURL}${url}`, fetchOption).then((response) => response.json()).then(function (result) {
            _logMessage(result);
...
async function _init(apiKey, engine) {
    try { // Check current network id and dapp network id registered on dashboard
        let getDappAPI = `${baseURL}/api/${
            config.version
        }/dapp`;
        fetch(getDappAPI, getFetchOptions("GET", apiKey)).then(function (response) {
            ...
            if (dappResponse && dappResponse.dapp) {
                ...
                if (dappResponse.log) {
                    ...
                    eventEmitter.emit(EVENTS.BICONOMY_ERROR, formatMessage(RESPONSE_CODES.DAPP_NOT_FOUND, `No Dapp Registered with apikey ${apiKey}`));
                }
            }
        }).catch(function (error) {
            eventEmitter.emit(EVENTS.BICONOMY_ERROR, formatMessage(RESPONSE_CODES.ERROR_RESPONSE, "Error while initializing Biconomy"), error);
