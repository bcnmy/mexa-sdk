...
// import ERC20ForwarderClient and PermitClient here and initialize them on init 
    ...
    try {
      if (!signer || typeof signer != "string") {
        let response = formatMessage(
          ...
          "signer parameter is mandatory and should be of type 'string'"
        );
  ...
  return new Promise(async (resolve, reject) => {
    if (rawTransaction) {
      ...
      if (decodedTx.to && decodedTx.data && decodedTx.value) {
        ...
        message.txGas = decodedTx.gasLimit.toString()
          ? decodedTx.gasLimit.toString()
          : 0;
          ...
          if (!engine.isLogin) {
            ...
            if (signature) {
              ...
              relayerPayment.token =
                config.DEFAULT_RELAYER_PAYMENT_TOKEN_ADDRESS;
              relayerPayment.amount = config.DEFAULT_RELAYER_PAYMENT_AMOUNT;
 ...
 */
Biconomy.prototype.withdrawFunds = function (
  receiverAddress,
  withdrawAmount,
  cb
) {
    ...
    try {
      getWeb3(engine).currentProvider.send(
        ...
        function (error, response) {
          _logMessage(
            `User signature for payload id ${DEFAULT_PAYLOAD_ID} is ${response.result}`
          );
          if (error) {
            if (cb) {
              cb(error);
              ...
              .then(function (response) {
                if (response) {
                  ...
                  let result = formatMessage(
                    RESPONSE_CODES.SUCCESS_RESPONSE,
                    response.log
                  );
                  ...
                  let error = formatMessage(
                    RESPONSE_CODES.ERROR_RESPONSE,
                    `Unable to get response for api ${withdrawFundsUrl}`
                  );
                  if (cb) cb(error);
                  reject(error);
                }
    ...
    if (decoderMap[to] || decoderMap[config.SCW]) {
      ...
      if (api.url == NATIVE_META_TX_URL) {
        if (metaTxApproach == TRUSTED_FORWARDER) {
          ...
          // right gasLimit needs to be passed or else server side estimation flow will pass on this gasLimit 
          const request = (
            await buildForwardTxRequest(
              account,
              to,
    ...
    _getUserAccount(engine, undefined, (error, response) => {
      ...
      _getUserContractWallet(engine, account, (error, userContract) => {
        if (
          userContract &&
          account &&
          account.toUpperCase() == userLocalAccount.toUpperCase() &&
      ...
      engine.sendAsync(
        ...
        function (error, response) {
          if (error || (response && response.error)) {
            ...
            fetch(
              `${baseURL}${getUserContractPath}?owner=${address}&&networkId=${networkId}`,
              getFetchOptions("GET", engine.apiKey)
            )
              .then((response) => response.json())
              .then(function (data) {
    ...
    fetch(`${baseURL}${userLoginPath}`, fetchOption)
      ...
      .then(function (data) {
        if (data) {
          ...
          if (
            data.flag &&
            data.flag == BICONOMY_RESPONSE_CODES.ACTION_COMPLETE
          ) {
  ...
  if (receipt) {
    if (receipt.status) {
      await _getUserContractWallet(
        ...
        (error, userContract) => {
          if (!error && userContract) {
            ...
            engine.isLogin = true;
            eventEmitter.emit(
              EVENTS.LOGIN_CONFIRMATION,
              "User Contract wallet created Successfully",
              userContract
            );
          }
        }
      ...
      eventEmitter.emit(
        EVENTS.BICONOMY_ERROR,
        formatMessage(
          RESPONSE_CODES.USER_CONTRACT_CREATION_FAILED,
          "User Contract wallet creation Failed"
        )
      );
    }
    if (loginInterval) {
...
Biconomy.prototype.login = async function (signer, cb) {
  return new Promise(async (resolve, reject) => {
    ...
    if (engine.status != STATUS.BICONOMY_READY) {
      let response = formatMessage(
        RESPONSE_CODES.BICONOMY_NOT_INITIALIZED,
        "Biconomy SDK is not initialized properly"
      );
      if (cb) cb(response);
      return reject(response);
    }
...
Biconomy.prototype.getUserContract = async function (userAddress) {
  ...
  if (!userAddress) {
    throw new Error(
      formatMessage(RESPONSE_CODES.INVALID_DATA, "Please pass a user address")
    );
  }
  if (this.isLogin) {
    ...
    if (userAddressFromStorage && userAddress) {
      if (userAddressFromStorage.toLowerCase() === userAddress.toLowerCase()) {
        response = formatMessage(
          RESPONSE_CODES.SUCCESS_RESPONSE,
          "User Contract Wallet address fetched successfully"
        );
      ...
      if (!response) {
        ...
        if (userContract) {
          ...
          response = formatMessage(
            RESPONSE_CODES.ERROR_RESPONSE,
            "Unable to fetch User Contract Wallet"
          );
        }
      }
    }
    ...
    response = formatMessage(
      RESPONSE_CODES.USER_NOT_LOGGED_IN,
      "Please login to biconomy first"
    );
  }
  return response;
};
